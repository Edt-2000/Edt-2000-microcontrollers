...
public class Kontrol
{
    ...
    public event EventHandler<KontrolEventArgs> OnMessage;
    ...
    {
        _output.Send([0x90, 0x5B, IsOn(type, 0x01)], 0, 3, 0);
        _output.Send([0x90, 0x5C, IsOn(type, 0x02)], 0, 3, 0);
        _output.Send([0x90, 0x5D, IsOn(type, 0x04)], 0, 3, 0);
        _output.Send([0x90, 0x5E, IsOn(type, 0x08)], 0, 3, 0);
        _output.Send([0x90, 0x5F, IsOn(type, 0x10)], 0, 3, 0);
    }
        ...
        if (x == 0x90)
        {
            if (y < 0x18)
            {
                ...
                if (mode < 3 && z != 0)
                {
                    ...
                    _output.Send([0x90, (byte)(channel + 8), IsOn(Channels[channel].Mode, Mode.One)], 0, 3, 0);
                    _output.Send([0x90, (byte)(channel + 16), IsOn(Channels[channel].Mode, Mode.Two)], 0, 3, 0);
                    _output.Send([0x90, (byte)(channel + 0), IsOn(Channels[channel].Mode, Mode.Four)], 0, 3, 0);
                }
                if (z == 0x7F)
                {
                ...
                OnMessage?.Invoke(this, new KontrolEventArgs(channel, Channels[channel]));
            }
