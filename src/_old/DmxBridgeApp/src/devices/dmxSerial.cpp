...
#define DMX_SERIAL_INPUT_PIN GPIO_NUM_15  // pin for dmx rx
#define DMX_SERIAL_OUTPUT_PIN GPIO_NUM_16 // pin for dmx tx
...
QueueHandle_t DmxSerial::dmx_rx_queue;

SemaphoreHandle_t DmxSerial::sync_dmx;

DmxState DmxSerial::dmx_state = DMX_IDLE;

uint16_t DmxSerial::current_rx_addr = 0;
...
void DmxSerial::Initialize(DmxDirection direction)
{
    ...
    if (direction == output)
    {
        ...
        // create send task
        xTaskCreatePinnedToCore(DmxSerial::uart_send_task, "uart_send_task", 1024, NULL, 1, NULL, DMX_CORE);
    }
    else
    {
        ...
        // create receive task
        xTaskCreatePinnedToCore(DmxSerial::uart_event_task, "uart_event_task", 2048, NULL, 1, NULL, DMX_CORE);
    }
}

uint8_t DmxSerial::Read(uint16_t channel)
{
...
void DmxSerial::ReadAll(uint8_t *data, uint16_t start, size_t size)
{
    // restrict acces to dmx array to valid values
...
void DmxSerial::Write(uint16_t channel, uint8_t value)
{
    // restrict acces to dmx array to valid values
    if (channel < 1 || channel > 512)
    {
...
void DmxSerial::WriteAll(uint8_t *data, uint16_t start, size_t size)
{
    // restrict acces to dmx array to valid values
    if (start < 1 || start > 512 || start + size > 513)
    {
...
void DmxSerial::uart_send_task(void *pvParameters)
{
...
void DmxSerial::uart_event_task(void *pvParameters)
{
    uart_event_t event;
